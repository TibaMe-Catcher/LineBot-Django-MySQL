...
def handle_text_message(event):
    ...
    if text == '@Hey, Catcher@':   # 點擊圖文選單，出現@Hey, Catcher@ → 回傳關心用語01 → 要自拍照
        ...
        func.sendText6(event)
    elif text == '@小小驚喜@':      # 點擊圖文選單，出現@小小驚喜@     → 回傳好聽的音樂或是美食資訊
        func.sendText5(event)
    elif text == '_今天_':          # 點擊選單，出現_今天_
        func.sendText7(event)
    elif text == '_一周_':          # 點擊選單，出現_一周_
        func.sendText8(event)
        ...
        # 呼叫模型比對文字是否有憂鬱，1是憂鬱，2是健康
        text_score = int(func.svm_test(text))

        # 回應使用者的對話
        if text == ["沒有"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的，已經幫你紀錄，可以點擊"心情回顧"追蹤心情天氣喔~'))
        elif text == ["沒有啦"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的，請點擊"心情回顧"，來追蹤自己的心情天氣!!'))
        elif text == ["沒有了"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='OK，已經幫你紀錄，請點擊"心情回顧"追蹤心情天氣~'))
        elif text == ["無"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的，已經幫你紀錄，可以點擊"心情回顧"追蹤心情天氣喔~'))
        elif text == ["no"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='OK，已經幫你紀錄，可以點擊"心情回顧"追蹤心情天氣喔~'))        
        elif text == ["0"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好勒，已經幫你紀錄，可以點擊"心情回顧"追蹤心情天氣喔~'))
        elif text == ["沒了"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='OK，請點擊"心情回顧"追蹤你的心情天氣~'))         
        elif text == ["冇"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的，快點擊"心情回顧"追蹤心情天氣喔~'))                 
        elif text == ["沒"]:
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='OK，已經幫你紀錄，可以點擊"心情回顧"追蹤心情天氣喔~'))                
        else :
            line_bot_api.reply_message(event.reply_token, TextSendMessage(text='好的，還有什麼可以跟我分享的嗎?可以再多說一點'))

        # 存進資料庫
        unit = Diary.objects.create(diary_id = message_id, text = text, diary_uid=diary_uid, text_score=text_score)
...
def handle_image_message(event):    
    ...
    # 呼叫模型比對照片的情緒
    picture_score = func.cv_test(filename)
